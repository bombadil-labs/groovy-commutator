#!/usr/bin/env python3
# Run-3 variant of ../evaluate_jev_probe.py: identical scoring, accepts mode semantic_codebook, reads ../truth_private.json.
...
def main():
    ...
    truth=json.loads((ROOT.parent/'truth_private.json').read_text())
    path=Path(args.responses); path=path if path.is_absolute() else ROOT/path
    rows=[json.loads(x) for x in path.read_text().splitlines() if x.strip()]
    ok=[r for r in rows if r.get('status')=='ok' and r['mode'] in ('semantic','semantic_codebook')]
    by=defaultdict(list)
    ...
    (ROOT/'jev_codebook_evaluation.json').write_text(json.dumps(report,indent=2,sort_keys=True)+'\n')
