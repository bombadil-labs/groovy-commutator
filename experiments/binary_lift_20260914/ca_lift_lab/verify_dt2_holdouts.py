"""Independent cropped-source audit of DT2 certificates and selected physical G.
...
import numpy as np
ROOT=Path(__file__).resolve().parent;RUN=ROOT/'runs/dt2_holdouts_36'
ZERO=1<<18;N=2048
...
def lookup(tx,ks):
    ...
    return v[pos]
def tag(r):return(r['rule'],r['mask'],r['shift'],r['reference'],tuple(r['order']))
...
def main():
    ...
    summary=json.loads((RUN/'summary.json').read_text());assert summary['complete']and len(rows)==29376
    indexed={tag(r):r for r in rows};assert len(indexed)==len(rows)
    for r in rows:
        m=mirror(r['rule']);q=r['reference'];mq=-1 if q<0 else q^1
        other=indexed[(m,r['mask'],-r['shift'],mq,tuple(r['order']))]
    ...
    for r in sorted(rows,key=tag):
        ...
        selected.setdefault(('geometry',r['reference'],r['shift'],tuple(r['order'])),tag(r))
    ...
    for r in sorted(rows,key=tag):
        prefix=tag(r)[:4]
        if prefix!=last:data=arrays(*prefix);last=prefix
        ...
        for name,cert in r['witnesses'].items():
            ...
            for p in (p0,p1):
                ...
                actual_key=one_key(data['uc']if probe else data['ac'],sample,order,phase);assert actual_key==key,(tag(r),name,'key')
                if probe:
        ...
        bk=keys(data['ac'],order);kb=keys(data['bc'],order);phases=[order.index(0),order.index(1)];pk=keys(data['uc'],order)[:,phases];bv=data['flip'][:,order]
    ...
    counts.update(status='passed',seconds=time.monotonic()-start,verifier_sha256=hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),scope='Every negative certificate; full cropped-source decisions across each passing rule/family/mode, each geometry and each mask/ref/sign represented by faithful recipes; direct physical G at every passing checked branch')
