...
All1,360 one-cell instruction/data edits agree with independent edited-native execution. This does not preserve fixed-source correction semantics automatically. All80 data-only edits recover on the tested odd rings, whose baselines extinguish, but a persistent full-lattice witness refutes general repair: a single top zero in the valid pair(1,1) spreads to radius t under Rule128. Flipping top leaf bit7 gives the corresponding expanding instruction-error witness.

The physical cap is exact; arbitrary semantic program revision and maintained repair remain separate questions. No Class-IV, novelty or universal-program claim is established.
