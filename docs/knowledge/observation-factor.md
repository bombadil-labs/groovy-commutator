# Factor existence differs from candidate-law disagreement

For a declared family X invariant under deterministic T=F^q and observation P, a whole-state factor B on P(X) exists exactly when equal current observations imply equal next observations: ker(P) is contained in ker(PT). Then B(P(s))=P(T(s)) is well defined and unique on the image. Locality, radius and off-image extension are additional questions.
