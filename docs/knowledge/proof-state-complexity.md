# Proof-state complexity of an erased distinction

A proposed quantity, not an established invariant: the minimum additional state
or representation complexity needed to restore an exact closure or permanence
...
This is deliberately **not** claimed to be a universal complexity hierarchy.
It is one controlled sequence of proof grammars for one frontier, under one
observer and one source language. The program measures lower bounds and
representation barriers; it does not prove that a minimum of this kind is
well-defined, nor that it would be independent of the representation used to
express the witness. Four proof states is an authorized budget, not a
mathematically privileged bound, and exhausting the ladder says nothing about
what a qualitatively different proof object — an invariant, an order-theoretic
abstraction, another exact quotient — might certify.
