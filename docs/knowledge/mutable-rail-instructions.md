...
Inherited instruction edits commute with the lift. Editing a newly introduced table can change the new system's relation to its previous source; it remains a valid source for another lift. Exactly 64 appended tables preserve unchanged old dynamics on the fixed rails. This is a scoped action condition, not a limit on allowable source routing programs.

The remaining fixed structure is the order and number of stages, their axis assignments, protected role tags, and the interpreter's hold/copy policy. Editing arbitrary syntax, reducing the layout budget, and generating the roles autonomously remain separate questions. No Class-IV criterion was used.
