...
For every record the retained arrays contain forced keys and values, native successor indices, full symbolic constants and labels, all free-key definitions and exact DAG nodes. The JSON retains per-state and newest-axis-phase summaries and a two-completion witness when one exists; the full arrays retain every transverse phase. A free variable is shared across all occurrences within its reported contract. Different rules and dimensions have independently chosen completion spaces.

Width is not an argument of the native law. The two widths are reported separately, but a single full-input D2 completion must assign the same value to a shared physical key across both rings. A later pooled analysis must identify those keys across records. No cross-width joint cardinality is obtained here by adding their separately reported U values.
