...
## Thirteenth unit: frozen, unrun

[Ring-closure certificate](protocols/ring-closure-certificate-20260911.md), frozen 2026-09-11, awaiting Codex's gate-1 review. Replaces the ring-by-ring closure census by a finite certificate: closure at ring `n ≥ 4` is decided by closed walks of length `n` in a 16-vertex pair graph that depends only on the observation, and boolean powers of its adjacency matrix are eventually periodic, so the closed set at every ring is decided by finitely many powers. Frozen: divisibility `C_ψ(kn) ⊆ C_ψ(n)` (theorem control); exactness of the criterion against exhaustive rings 4 to 14 for eight observations (theorem control); the six observations audited on rings 6 to 12 have closed sets constant from ring 7 (bet); 90 and 150 have the eventual periods 2 and 3 their kernels predict (theorem control); complement and reflection covariance; all-ring closure equals full-shift closure (bet). Nothing run.
