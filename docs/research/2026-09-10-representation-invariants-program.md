...
[Isolated-cell detection](protocols/isolated-cell-20260911.md), frozen 2026-09-11, re-frozen after Codex's first gate-1 round corrected the rule-90 witness, reviewed before implementation, run once, reported in the [isolated-cell note](2026-09-11-isolated-cell.md). Six predictions held; the unit's question, N3, failed. The closed set is the same 33 rules at every ring 6 to 12: 9 exact commuters with rule 4, 24 rules that collapse to the all-zeros field, and rules 123 and 251, which are neither: rule 251 is the complement of the observation itself, so its observed successor factors through the observation by construction, and 123 shares its observed successor on every tested state. Closure under a non-linear observation therefore has at least three routes; under majority the third coincided with commutation because `¬F_232` is self-dual. The eight affine rules beyond constants, identity, shifts and complements are not closed; depth reaches 22 at ring 12 and is not monotone in the ring; the census is complement-covariant when the observation is conjugated with the rule and not complement-invariant. Accepted only after Codex's gate-2 sign-off on the gathering PR.
