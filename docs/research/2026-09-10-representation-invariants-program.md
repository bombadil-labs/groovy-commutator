...
## Tenth unit: frozen, unrun

[Complement of the observation](protocols/complement-observation-20260911.md), frozen 2026-09-11, awaiting Codex's gate-1 review. Three further non-linear, non-self-dual observations, rules 32, 200 and 22, with their conjugates. Frozen: the complement rule `¬ψ` closes by construction and, for these observations, lands in the residual class of closed rules with a non-constant factor that is not the rule; every rule in that class is observationally equivalent to `¬ψ` (the unit's falsifiable question); observational equivalence is decided by five-cell words and so is the same at every ring `n ≥ 5`, which certifies the ninth unit's rule-123 identity for all rings; named commuters and collapses; a depth annex at rings 13 to 16 on the record rules, with one bet (majority's record depth grows past 16). Nothing run.
