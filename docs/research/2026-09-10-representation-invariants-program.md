...
## Eleventh unit: frozen, unrun

[Wiring as a transformation](protocols/wiring-dilation-20260911.md), frozen 2026-09-11, awaiting Codex's gate-1 review. Opens a fourth transformation type: a change of interaction graph with the rule table fixed, prompted by B. Rose's interaction-graph preprint (CargoCultResearch, 2026). Neighborhood dilation by `d ∈ {2, 3}` on rings 6 to 12, with two declared transports: whole-contract dilation (rule and observation dilated together) and partial dilation (rule only). Frozen: when `gcd(d, n) = 1` whole-contract dilation is conjugation by a site relabeling, so every census and the result-1 commutator classification are exactly preserved (theorem control); partial dilation changes the closed set at every odd ring for observations 232 and 4, with the observation rule itself leaving the commuter set, and equals the standard census under an observation rewired to the inverse offset (consistency control); when `d` divides `n` the ring splits into `d` interleaved rings and the census equals the small-ring census, depth included. Nothing run.
