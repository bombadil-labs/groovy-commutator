...
- **B4, Research026 observer family.** The static family of all nonconstant Boolean functions of 2-cell and 3-cell blocks is closed under input complement and under input reversal, output-complement pairs are preserved, and the 12-ring block alignment survives reflection for both block sizes. By conjugacy, every per-rule summary of the [possibility-frontier census](2026-09-08-possibility-frontier.md) is therefore identical for a rule, its complement-conjugate, and its mirror. The per-rule tables of that census are not in the repository, so this is a deduction from a finite check, not a numerical audit; if those tables are regenerated, the equality is a one-line test.

## Reading for the program

The higher-block recoding is close to free: it never costs radius, it buys at most one unit, and it does so in 9 of 674 cells. For the primitives program this is the first calibration of an equivalence notion: two constructions that differ by a 2-block recoding differ by at most one unit of cap radius and by nothing in closure or in the commutator. A learner that counted such a recoding as a new primitive would be paying for a coordinate change.
