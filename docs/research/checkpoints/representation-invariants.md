...
- Evaluated and reported: `2026-09-13-closed-violation-depth-three.md`, canonical result
  `results/closed_violation_depth_three_20260913.json`, generated by Myk running
  the pinned verifier off GitHub Actions on a separate machine, from the exact
  reviewed head `320d3ae`, and integrated directly (merged `bc9131d`). A
  concurrently-running independent execution in this session (70.9 minutes,
  inside the protocol's 60-to-100-minute expectation, bound ~3 h) completed
  byte-identical to Myk's file before this session's own second execution
  finished, so the second was stopped rather than run to an already-settled
  conclusion — two-environment determinism confirmation, not a single-machine
  self-rerun. **Process note, preserved not rewritten:** both runs started
  independently and concurrently from the same pinned head without prior
  coordination; once noticed, the gathering PR recorded the situation so the
  two runs would be compared rather than produce competing sub-PRs or
  registrations. 874 deep depth-three pairs, 56 `FS³` controls, all 2,048 pairs at
  each of depths one and two, **zero matrix powers at any depth**.
- Outcome: **Q1, Q3 and Q6 held in every part; Q2's theorem controls and bets (a)
  and (b) held while bet (c) failed; four of Q4's seven period bets failed; Q5's
  Corollary B control held and its bet failed under two of its three
  observations.** Lemma A gives `D^h_ψ^∞ = {r : V^h_cl = ∅}` at every depth, so
  the all-ring depth-three sets (sizes 206, 116, 116, 187, 242, 254, 246) and
  gaps (8, 10, 10, 6, 8, 4, 0 rules under 232, 4, 32, 200, 22, 90, 150) are
  decided without the 65,536-vertex powers the eighteenth unit ruled out.
- What the failures establish: the lying ring window moved. At depth two 22 was
  the one observation whose rings-3-to-14 window overstated the all-ring gap; at
  depth three **22's window is exact and 200's overstates**, five rules (41, 97,
  169, 225, 233) leaving at ring 16. The eventual periods 1, 2, 2, 1, 1, 2, 6 do
  not carry up from the certified depth-two 1, 2, 2, 1, 12, 4, 6 — collapsing
  under 22, halving under 90, persisting at 2 under 4 and 32 but on a different
  population. **Do not interpolate a ring-window or period claim across depth
  levels**; carry the level with the claim. Post hoc on one observation: 200's
  five late rules are exactly its eventually-out rules whose single carrying
  component has no diagonal vertex.
- Not decided, and not to be inferred: **no ring certificate at depth three.**
  The onset beyond which the eventual pattern holds is undetermined and no bound
  is claimed, so individual rings between 14 and that onset remain open. Nothing
  about depth four on any ring or on the full shift; `FS³` is read with its hash.
- **Process note, stated plainly: this unit needed three review-driven
  corrections before any evaluation ran, and that is the review process working.**
  (i) #219 committed the verifier with no result. (ii) #220 fixed a determinism
  defect — wall-clock timing serialized into the canonical JSON, which would have
  failed this unit's own byte-for-byte replay — found by a prior evaluation
  attempt that was stopped before writing anything. (iii) Codex's deeper
  pre-evaluation review then found a genuine **mathematical error in a frozen
  theorem control (I1)**: reflection transports `R_r` as an *equal* set, not
  negated, because the violating walk's ends swap and cancel the cyclic-class
  sign flip; the reviewer withdrew its own first-round remark accepting the old
  convention. That forced a protocol re-freeze (`3cfac14`), renewed gate 1, and
  #221, which removed the `negate_residues` parameter outright and added a
  self-test assertion sharp enough to fail on its regression. Two of the three
  corrections were caught **before** any of the 71 minutes was spent. The pattern
  worth keeping: a theorem control that only runs on the expensive source domain
  is a control the cheap tier cannot protect, so give each one a sharp cheap
  witness.
- **Gate 2 accepted.** Codex (OpenAI GPT-5.6 Sol) requested changes at head `e2f1cb9`
  (G2-B1 through G2-B5: an all-ring-vs-per-ring wording error in the
  depth-level-arithmetic knowledge entry, unfenced post-hoc causal language in
  three records, run-provenance misattribution of the 70.9-minute timing to
  the wrong execution, stale draft/"protocol only" gathering-PR metadata, and
  a missing Program-table row plus an unrecorded observation-102 serialization
  omission), all addressed records-only in sub-PR #226 with no scientific or
  verifier change, then **approved gate 2 on exact head `a12b95c`** and merged
  #218 to `main` itself as `33ddae7`, per AGENTS.md's reviewer-merges rule.
  AGENTS.md's established-results list is updated in the same acceptance pass
  as this checkpoint.
- Candidate next units: (a) the **depth-three onset** `N³_ψ`, the one thing this
  unit's method cannot reach — it needs the power sequence, so it inherits the
  eighteenth unit's cost question at 65,536 vertices and should be scoped before
  being frozen; (b) **full-shift depth four** for the twenty deep rules, still
  parked pending Myk's cost decision under the 2026-09-12 CI-cost boundary;
  (c) a **structural account of the non-diagonal witnesses**, testing whether the
  anchoring/lateness correlation found post hoc under 200 recurs under other
  observations and at other depths, since it failed to appear under 232;
  (d) applying Lemma A at **depth four** on rings, which needs no new theory but a
  `4^10`-vertex graph.
