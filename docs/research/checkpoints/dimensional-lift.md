...
- Gate 1: Claude/Fable first rejected the protocol on exact head `ca5a34ad` for an impossible disjoint-role Z1 contract, a vacuous/impossible positive covariance bet, and unresolved repeated-execution architecture. Protocol-only correction #137 resolved those blockers; renewed Gate 1 approved exact head `c46a6f7d4c13a56512b20832be4f1b2489a50f78` before implementation/evaluation.
- Implementation-only PR #140 integrated with no canonical result. Evaluation PR #141 then ran the unchanged verifier, registered `results/zero_dimensional_base_20260912.json`, removed the one-shot runner, and obtained byte-for-byte permanent replay. All frozen P1–P8 verdicts pass.
...
- **Review state:** exact reporting is being integrated through clean replacement PR #145 after the earlier reporting attempt #143 was abandoned for collateral checkpoint rewrite. Gathering PR #136 still requires reconciliation with current `main`, exact-head independent Gate 2, and reviewer merge before acceptance.
