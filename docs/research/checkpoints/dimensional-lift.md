...
## Checkpoint 2026-09-12: zero-dimensional base audit completed on gathering branch; gate-2 pending

Read `docs/research/2026-09-12-zero-dimensional-base.md`. This unit audits the bottom two law-as-state selector rungs after the first Gate-1 review exposed an impossible disjoint-role Z1 interpretation and a vacuous positive covariance bet.

- Gate 1: Claude/Fable withheld approval on `ca5a34ad`, requiring explicit address/program role overlap, replacement of the covariance bet with the cycle-type/antipode obstruction, and removal or pre-freezing of the repeated-execution architecture. Corrections merged in PR #137; Gate 1 then approved exact gathering head `c46a6f7d`.
- Implementation-only PR #140 integrated at `2780d131` with no canonical result. Evaluation PR #141 then produced `results/zero_dimensional_base_20260912.json`; all frozen P1–P8 verdicts pass, source hashes are registered, and the permanent workflow replays the result byte for byte.
- Complete binary 0D floor `{0,204,51,255}` has Groovy constants `(0,0,1,1)` under the accepted affine theorem/check.
- The single-layer selector overlap lower bound `max(0, k + 2^k - |P|)` is attained: zero overlapping roles at Z0; exactly two at Z1, where `W` and `E` are deliberately both address and program cells.
- The frozen Gray-code family deduplicates to 16 layouts. Every Z1 selector has all nine physical inputs essential, degree four, and exactly six quartic terms; independent scalar/packed evaluators and two ANF paths agree. Degree/full essentiality are generic multiplexer facts; the six quartics are only the layout-independent fingerprint of two dual-role cells.
- Every frozen Gray layout has identity-only intersection between induced shell `D4` address actions and the ECA group generated by mirror/address-complement, matching the pre-run fixed-point/cycle-type and antipode proof. This is a negative geometry result, not a preferred-layout result.
- Center-only axial dynamics agree with `f^d`; rule 51's dimension-parity alternation is separate from its replication-interface failure (`u_51=NOT` fixes no bit). The accepted editable-routing family has 16 native routing bits at `d=2`, strictly not compression of arbitrary 512-bit 2D laws.
- Repeated stateful execution was deliberately split out before implementation. Do not infer a dynamical dimensional lift, natural/unique geometry, arbitrary 2D→3D shell encoding, intrinsic dimension, self-assembly, endogenous control, spacetime emergence, physics, metaphysics, or any `8n+1`/prime connection.
- **Review state:** reporting note/catalog are in reporting PR #143; dimensional Program/knowledge integration and independent exact-head Gate 2 remain required before the gathering PR #136 may merge to `main`.
...
- Frozen protocol at 9e5de20f; implementation bf62df70 precedes evaluation.
  First execution passed; no corrections or deviations. Same 66 ordered axial
  laws, observation J=(T_x,T_y).
...
- Appending selector word 172 with separate guard Qd=(204,172,...)
  lifts every native source program/data field. Both retention and state-gated
...
boundary/individuation question remains parked.
