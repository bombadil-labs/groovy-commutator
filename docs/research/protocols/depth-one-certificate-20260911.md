...
Cost: exhaustive depth-one checks for 8 observations × 12 rings × 256 rules at up to `2^14` states with three applications of the rule, a few minutes; per `(ψ, r)` a 256-vertex graph from 16384 seven-cell blocks and its boolean powers to the first repeat, 2048 graphs. Depth two would need 4096-vertex graphs per rule and is out of scope.

**Power cap and censoring contract (frozen before implementation, after Codex's gate-1 round).** For each `(ψ, r)` the verifier computes boolean powers `A^0, A^1, …` and stops at the first repeat or at the cap `A^{1024}`, whichever comes first; the cap is a constant of the verifier and is recorded in the canonical artifact's parameters and in the CI replay. A pair `(ψ, r)` whose powers have not repeated by the cap is recorded as **censored**: L3 claims no `(k, p)` for it, its membership in `D_ψ(n)` is decided only for `4 ≤ n ≤ 1027` (the powers actually computed) and marked open beyond, and any observation-level L4, L5 or L7 all-ring verdict that depends on a censored rule is itself recorded as censored, not inferred. The rings-4-to-14 exactness check L2 does not depend on the cap. No larger cap or rescue computation is introduced after the run; a censored cell is a reported limit of this unit. Memory is bounded by the stored powers, at most `1025 × 256 × 256` bytes per pair, released between pairs.
...
**First round (2026-09-11, before implementation).** Codex reviewed revision `d6e6b1c` (integrated head `4f7f7cc`), found the depth-one criterion, radii, vertex and edge sizes, divisibility, the bet/control split of L4 and the theorem/reported split of L5 sound, and required one binding clarification: the certificate's computational failure and censoring contract, since first repeats of 2,048 rule-dependent 256 × 256 boolean-power sequences have no a-priori bound and the protocol had no cap or scored outcome for a non-repeat. Option (b) of Codex's two was chosen and is frozen in Section 4: a deterministic cap of 1024 powers, recorded in the artifact and CI, with censored cells reported and never inferred through. Observations, ring domain, criterion, predictions and artifacts are unchanged. Gate 1 re-requested on the corrected head.
