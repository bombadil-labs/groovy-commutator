...
Current progress: **7 of 8 complete**. The primary session's six issues and the parallel #63 protocol are completed; the latest is [PR72](https://github.com/bombadil-labs/groovy-commutator/pull/72) (2026-09-10). Both new censuses remain planned and unrun. #64 is the remaining reserved parallel-agent implementation.
...
| Order | Status | Issue and deliverable | Done when |
| --- | --- | --- | --- |
| 1 | Complete — [PR71](https://github.com/bombadil-labs/groovy-commutator/pull/71), 2026-09-10 | [#61](https://github.com/bombadil-labs/groovy-commutator/issues/61): shared closure/fiber account | A synthesis PR states the agreed observation, forward-invariant source family, cadence and candidate-law contract; distinguishes failure of a chosen law from nonexistence of a factor; includes the Rule255 counterexample; and adds the missing first-class knowledge provenance for Research022–028 and Research035. Coordinate the shared Program wording with the parallel workstream and preserve its unrelated findings. |
| 2 | Complete — [PR73](https://github.com/bombadil-labs/groovy-commutator/pull/73), 2026-09-10 | [#62](https://github.com/bombadil-labs/groovy-commutator/issues/62): archive the successor-set example | The already reproduced Rule110, derivative-observation, n=8 example has a runnable repository script, saved output and a research/knowledge account. Whole-field successors, sound local product approximations, correlations and weighting conventions remain distinct. Label the original run exploratory and retain the broader approximation question as open. |
| 3 | Complete — [PR74](https://github.com/bombadil-labs/groovy-commutator/pull/74), 2026-09-10 | [#65](https://github.com/bombadil-labs/groovy-commutator/issues/65): archive relabeling and transport | The local relabeling identity and transformed synchronous transport law are documented with their proof, a runnable version of the reproduced example, saved checks and provenance. Separate the exact identity from seeded exploratory checks; name the gate convention and preserve the unspecified rule-field lift as an open question. |
| 4 | Complete — [PR75](https://github.com/bombadil-labs/groovy-commutator/pull/75), 2026-09-10 | [#66](https://github.com/bombadil-labs/groovy-commutator/issues/66): cross-constructor inventory | Saved-source extraction records the three declared sets, all pairwise intersections and the intrinsic counterpart of the finite role census, with an explicit substrate/constructor column. Keep the four nonlinear gradient sources visible. State the intertwining equation precisely and leave the conserved-density search as a separate, unrun direction with no automatic loop-parity requirement outside its grammar. |
| 5 | Complete — [PR76](https://github.com/bombadil-labs/groovy-commutator/pull/76), 2026-09-10 | [#68](https://github.com/bombadil-labs/groovy-commutator/issues/68): publish the resonance distinctions | Concepts has a stable anchor, the four-row table and sourced examples. Family restoration and recovery of an undamaged trajectory remain distinct; endogenous establishment remains open. Update promotion metadata and the knowledge backlink while retaining the vision note's open evidence label. Research/site checks pass. |
| 6 | Complete — [PR77](https://github.com/bombadil-labs/groovy-commutator/pull/77), 2026-09-10 | [#67](https://github.com/bombadil-labs/groovy-commutator/issues/67): freeze the second-lift comparison | A complete protocol fixes the product-alphabet constructor, inherited family B, H128/H160 completions, depth/radius budgets, causal-window bounds, realizability checks, explicit recoding and locality costs, separate ambient control, feasibility estimate and independent verification plan. State the existing completion-independent whole-field theorem before measurement. Register implementation/evaluation as a named planned experiment linked from the protocol and Program. No new census result is required to complete this agreed protocol-writing scope. |
| 7 | Complete — [PR72](https://github.com/bombadil-labs/groovy-commutator/pull/72), 2026-09-10 | [#63](https://github.com/bombadil-labs/groovy-commutator/issues/63): freeze the hidden-state static channel | A complete protocol fixes the source/observer/ring census, no-op/flip action set, uniform-microstate weighting, primary and immediate-observation horizon variants, idealized limiting readout, uniform-action information versus optimized capacity, floating-point brackets and rigorous interval certificates, and both exact zero-information controls. Register implementation/evaluation as a named planned experiment linked from the protocol and Program. Predictions remain unrun. |
| 8 | Open | [#64](https://github.com/bombadil-labs/groovy-commutator/issues/64): implement the conservative suffix learner | The agreed online arm and prequential/held-out harness are implemented and checked on explicit traces. Predict before recording; refine only on a wrong definite prediction; retain raw history across rebuilds; start fresh held-out histories with frozen table/depth; advance the harness once per observation. Charge actual storage and key/rebuild work. Merge/coarsening and the oracle arm remain deferred; no eventual-determinization or optimality claim. Freeze the implementation before any new research evaluation. |

### Dependencies and scheduling

- #61 is the recommended first shared foundation. #62, #65 and #66 can otherwise be prepared independently; their numbers in the list are a default work order, not mathematical dependencies.
- #68 needs the agreed vocabulary and sourced examples. It can cite the already proved completion-recoding consequence discussed in #67; it does not need to wait for a second-lift experiment.
- #67's explicit preceding requirement, the frozen gradient intervention audit, is satisfied by PR69. Its next deliverable is the protocol. Check causal-window and computational costs before promising an exhaustive run.
...
Use `Closes #N` in the PR that fully delivers the agreed scope. Preparatory or partially addressing PRs only reference the issue. Update this checklist in the same closing PR with its link and completion date. A changed scope needs an explicit issue record, not an expanded definition of done hidden in implementation.

For #63 and #67, the signed scope is protocol drafting/freezing. A merged, complete protocol can resolve that proposal issue, provided its future implementation and run remain visibly registered as planned work with a stable link. Proposal completion must never be reported as an executed experiment or a new finding. #64 requires the working online implementation, not pseudocode alone.

Before closing #62 or #65, move their existing scripts and worked examples out of issue comments into durable repository artifacts. Preserve exploratory provenance; rerunning an existing example does not retroactively preregister it.

Use the existing review and CI gates, self-review explicitly as Codex, and merge when green under the standing workflow. Fresh implementation results and their limitations should be recorded before any evidence upgrade. This planning update closes no issues and starts no experiments.
