...
## Cross-width continuation breaks at width nine
...
## Frozen resonance-response bet fails on primary ring seven
...
24. **The natural six-bit touching-strip interface fails bounded local factorization.** On the exhaustive declared adjacent-strip reachable family for rings `n=6,7`, none of the 16 frozen `(A,B)` plus interface-bit masks admits an autonomous local factor at longitudinal radius `R=0,1,2` through the primary horizon; full P15 also fails through the longer stress horizon. J1/J2/J8 controls pass, and the canonical full-state `R=1` and `R=2` conflicts independently replay to causally relevant physical differences outside the retained four-row band. This closes one natural finite symbolic-interface attempt negatively, not all larger-radius, history-bearing, moving, nonlocal, or alternative representations.

25. **Complete two-lag interface history can retain enough information globally while radius two still fails.** On the frozen touching-strip family, full `P15,h=2,D2` is globally deterministic on rings 6 and 7 although every accepted local radius `R<=2` conflicts. Only 9/192 complete-history cells pass and all occur at `D2,h=2`; the other 183 have independently replayed global conflicts. This separates representation-level information loss from unresolved locality scale on the declared finite rings, without establishing a larger-radius or infinite-lattice factor.

26. **Cross-width two-lag determinism persists through width eight and fails at width nine.** For the frozen `D2,h=2` touching-strip masks `{11,13,15}`, widths 6/7 retain their accepted W1 passes and theorem/regression `R=3` passes; all three masks also pass both W1 and W2 at fresh width 8. At fresh width 9, all three masks fail W1 and therefore also fail W2, with independently replayed canonical conflicts. The pooled width-blind W3 table fails for all three masks; P15 has a canonical n=7 versus n=9 conflict. These are finite reachable-family results. They do not establish a width-nine threshold, arbitrary-width/full-shift/infinite-lattice failure, or intrinsic dimension.

27. **Transformed-homologue mutual transparency fails as a uniform primary response.** In the frozen touching Rule90 response audit, the primary ring-7 strict predicate fails at `k=2,3,4`: residual mass/span class signs are mixed and validity/return contrasts are all zero. The orbit-preserving placebo criterion is inapplicable because the actual strict predicate never passes. Ring-6 `k=4` is a declared small-family control only. This is an exact finite negative for one operational response hypothesis, not an absence theorem for all homology-sensitive dynamics.
...
## Completed execution: finite-width packing and transverse freedom

The [frozen protocol](protocols/transverse-freedom-20260911.md) and [result note](2026-09-11-transverse-freedom.md) separate finite column packing from independently growing transverse capacity. For a fixed width `w`, a strip is sitewise equivalent to a one-dimensional CA over the product alphabet `A^w`. For an unbounded independent strip family, any fixed target alphabet and fixed longitudinal expansion budget eventually fails by counting.

The inherited Rule32 correction family goes the other way: every finite-depth tuple is a deterministic image of the same binary source family, so its independent-state rate is at most one bit per longitudinal site. The bounded periodic diagnostic tests rings `n=6..12` and depths `h=0..4`. The first-image family sizes are 63, 128, 255, 512, 1023, 2048, 4095; the corresponding tuple images are 62, 128, 254, 512, 1022, 2048, 4094 at **every** measured depth. Because deeper tuples retain the shallower tuple as a prefix, the unchanged counts mean the whole-field equality partition is already saturated at `h=0` throughout this diagnostic. K/O and H128/H160 fiber/recoding controls all pass.

This is a necessary anti-packing/resource result, not an intrinsic-dimension theorem. The measured saturation is not extrapolated beyond `n=6..12` or `h=4`, and non-spatial memory can also violate a fixed capacity budget. The next dimensional criterion should therefore ask for a resource that survives finite packing under a uniform contract—candidate examples include transverse intervention independence, interaction topology, or translation symmetry in a new direction.
...
The [frozen protocol](protocols/interface-factor-20260911.md) and [result note](2026-09-11-interface-factor.md) test whether the interacting pair of adjacent finite Rule90 strips can be made locally autonomous by retaining the two strip vertex bits plus four natural interface-correction bits. The six coordinates are an invertible recoding of the retained four-row band and the interface bits vanish initially, so the test does not manufacture a positive result by discarding same-site information.
