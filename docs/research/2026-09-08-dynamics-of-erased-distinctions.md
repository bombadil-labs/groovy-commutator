...
## Parked proposal: representation empowerment census

The [frozen no-op/flip protocol](protocols/representation-empowerment-20260910.md) and its [registered experiment](../knowledge/representation-empowerment-planned.md) proposed a control question for the existing repertoire frontier. The [publication note](2026-09-10-representation-empowerment-protocol.md) records observer/cadence provenance and the exact-input capacity certificate. Implementation and numerical evaluation never began.

The 2026-09-22 [prospective disposition](2026-09-22-representation-empowerment-disposition.md) parks the census unrun. Its primary stable-future outcome includes the post-action observation, so identity has disjoint deterministic action rows and the maximum one-bit capacity for every rule. That exact endpoint result already separates capacity from identity's zero future repertoire. A new control study would need a delayed target, common physical cadence, action-phase controls, a named consumer and a cost baseline; the old protocol is not silently repaired.
...
## 2026-09-14: the phase-splice source-recoder frontier, integrated and still open

A thread of this Program developed on a branch between 2026-09-09 and 2026-09-10 and was then orphaned in an unreviewed draft PR: the phase-splice / bounded source-recoder line, which asked not when an erased distinction next becomes visible but what finite object would certify that it never does. It is now integrated, and the Program's account of it is the [terminus/synthesis note](2026-09-10-erased-distinctions-terminus.md).

What that thread contributes to the current account:

- Finite silence and all-time permanence need different proof objects. Research036's exact sofic criterion — the paired one-defect source language is sofic, sliding-block images stay sofic, and `X_{t+1} ⊆ union_{j<=t} X_j` with finite-prefix target safety gives an all-time certificate — is what permanence would need. It is now registered as a [knowledge entry](../knowledge/sofic-permanence-certificate.md).
- A censoring is a statement about a proof representation, not about the dynamics. Phase-splice MDD evaluation censored all twelve Rule 122/161 sentinels under its frozen wall; the identical candidates as fine-ECA CNF were tiny and exact. The Program adopts this as a standing methodological rule and it is registered [as its own entry](../knowledge/presentation-versus-dynamical-complexity.md).
- The thread climbed a controlled ladder of permanence-proof grammars — same-source recurrence, changed provenance, one phase cut, a two-cut diagnostic, and a bounded adaptive finite-state selector with `m <= 4` — and found no certificate on any rung. One global bit of left/right provenance phase is exactly negative across the full 22-seed / 170-question frontier. The ladder motivates the proposed but unestablished notion of [proof-state complexity](../knowledge/proof-state-complexity.md).

**Status of this frontier: open, not closed.** The terminus note recommends marking the Program complete/dormant for this frontier, but that recommendation is explicitly conditional on all 22 seed languages being exactly classified, and the count stands at **14/22** with **0/8** recovery seeds recovered and **0** certificates. A further exact-recovery campaign on 2026-09-14 added about 9.3 recorded solver-hours and decided no structural tuple; the [recovery-status note](2026-09-14-bounded-source-recoder-recovery-status.md) records exactly where it is stuck and the [open question entry](../knowledge/bounded-source-recoder-classification.md) carries the classification. Nothing here licenses reading the eight undecided seeds as negatives.

The frozen empowerment protocol and the conservative online suffix learner above remain separate historical/implemented work. The empowerment census was later parked unrun by its 2026-09-22 endpoint audit. The two-switch rail-selector rung is retained as a diagnostic only and is not promoted as an exact classification. The selector-state ladder is not to be resumed silently: enlarging `m`, enlarging `t`, or inventing another selector geometry would be a new research question under its own frozen protocol.
