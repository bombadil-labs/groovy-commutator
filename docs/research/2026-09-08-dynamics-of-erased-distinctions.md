...
The frozen empowerment census and the conservative online suffix learner above are untouched by this thread and remain separate planned/implemented work. The two-switch rail-selector rung is retained as a diagnostic only and is not promoted as an exact classification. The selector-state ladder is not to be resumed silently: enlarging `m`, enlarging `t`, or inventing another selector geometry would be a new research question under its own frozen protocol.
