...
def summarize(rows,class_by_rule):
    ...
    for w in WIDTHS:
        ...
        for p in POSITIVES:
            for c in controls:
                predictions[f'P3_w{w}_{p}_beats_{c}']=bool(selected[p]['metrics']['4']['all']['score']>selected[c]['metrics']['4']['all']['score'])
...
def run(archive,out):
    ...
    for w,r in sorted(selected):
        ...
        row=dict(rule=r,width=w,on_beam_control=True,forced_keys=len(table[0]),
                 ...
                 input_record=f'w{w}/rule{r:03d}/d2.json',elapsed_seconds=time.perf_counter()-tick)
